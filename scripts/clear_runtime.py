#!/usr/bin/env python3
"""Clear worker/split runtimes on the GPU rig.

Targets worker single-GPU ports from config plus split runtime ports from the
models catalog. Also clears split/model-load signal artifacts that commonly
wedge future loads, and reaps legacy runtime processes if any remain.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Clear worker/split runtimes on GPU rig")
...
remove_container_if_present() {{
  ...
  if [ "$kind" = "split" ]; then
    split_runtime_containers_removed=$((split_runtime_containers_removed + 1))
  else
    worker_runtime_containers_removed=$((worker_runtime_containers_removed + 1))
  fi
}}

for p in {ports_str}; do
  worker_name="llama-worker-gpu-$((p-11434))"
  remove_container_if_present "$worker_name" "worker"
done

for name in $(docker ps -a --format '{{{{.Names}}}}' | grep '^llama-split-' || true); do
  remove_container_if_present "$name" "split"
done
...
# Kill all non-brain llama-server host processes (workers/split runtimes), even if still parented.
for pid in $(ps -eo pid=,args= | awk '$0 ~ /llama-server/ {{print $1}}'); do
  [ -n "$pid" ] || continue
...
printf '{{"worker_ports_reclaimed":%s,"split_ports_reclaimed":%s,"worker_runtime_containers_removed":%s,"split_runtime_containers_removed":%s,"runtime_host_processes_killed":%s,"signals_removed":%s,"target_port_listeners_remaining":%s,"worker_ports_killed":%s,"split_ports_killed":%s}}\\n' \\
  "$worker_port_killed" "$split_port_killed" "$worker_runtime_containers_removed" "$split_runtime_containers_removed" "$runtime_host_processes_killed" "$signals_removed" "$target_port_listeners_remaining" "$worker_port_killed" "$split_port_killed"
"""

    print(f"Target worker ports: {worker_ports}")
    print(f"Target split ports: {split_ports}")
    print("Clearing worker/split runtimes on gpu host...")
    ...
    if not args.json:
        ...
        print("Clear runtime summary:", json.dumps(summary, indent=2))
    else:
